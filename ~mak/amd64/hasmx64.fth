...
: use32         \ -- ; force 32 bit segment
  1 seg-type !  reset
;

: use64         \ -- ; force 64 bit segment
  2 seg-type !  reset
;

\ ******************************
\ *S Generating new instructions
\ ******************************
\ *P Some processors, especially those used for embedded
\ ** applications, have processor-specific instructions that are
\ ** extensions to the AMD64 instruction set. In order to use these,
\ ** a few facilities are available.

: dxb            \ b -- ; lay byte
\ *G Lay a byte into the instruction stream. Use in the form:
\ *C   dxb $55
  0 ?prefix drop  C, reset  ;

: dxw            \ w -- ; lay 16 bits
\ *G Lay a 16-bit word into the instruction stream. Use in the form:
\ *C   dxw $55AA
  0 ?prefix drop  W, reset  ;

: dxl            \ l -- ; lay 32 bit long
\ *G Lay a 32-bit dword into the instruction stream. Use in the form:
\ *C   dxl $11223344
  0 ?prefix drop  L, reset  ;

: dxx            \ n -- ; lay 64 bit xword
\ *G Lay a 64-bit dword into the instruction stream. Use in the form:
\ *C   dxx $1122334455667788
  0 ?prefix drop  , reset  ;

: $             \ -- chere
\ *G Return the PC value of the start of the instruction.
  HERE  ;

: VFXASM_BIG	asm-on ;
: VFXCODE	HEADER  asm-on set-null ;
: END-CODE	null-op asm-off ;
