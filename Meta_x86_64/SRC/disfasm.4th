...
: BASE-EXECUTE ( i*x xt u -- j*x ) \ gforth
  ...
  BASE M@ >R BASE M! EXECUTE R> BASE M! ;
[THEN]      
...
: D# BASE M@ >R DECIMAL PARSE-NAME EVALUATE R> BASE M! ;  IMMEDIATE
...
: .HBY   ( 8B -- )     BASE M@ >R HEX  0  <#  # #  #>  TYPE R> BASE M! ;
: .$BU  ( 8B -- )      .$ .HBY ;
...
\ ABORT  ." ZZ=" H. EXIT
 ...
 9848 T" dw 9848h" \  T" cltq"
 9948 T" dw 9948h" \  T" cqto"
  CB48 T" lretq"
...
\  TR_FLG IF DUP CR ." ( "  HEX. ." ) "  DUP @ H.  CR .S CR KEY DROP  THEN
  ...
  IF CR TAB ." .byte 0x"  COUNT DUP H.-
	OVER + 7 + 7 ANDC OVER -
 	0 ?DO
 DUP 7 AND IF ." ,0x" ELSE CR TAB ." .byte 0x" THEN
\ ." ,0x"
 COUNT  H.-
 LOOP
  ...
  IF CR TAB ." .byte 0x"
        COUNT DUP H.-
	BEGIN 
	WHILE DUP 7 AND IF ." ,0x" ELSE CR TAB ." .byte 0x" THEN  COUNT DUP H.-
	REPEAT 
\	BEGIN DUP $F AND
\	WHILE  ." ,0x"  COUNT H.-
\	REPEAT
...
\    DUP H.
	...
	>R DUP
	IF   DUP NAME>  R> - 0=
	     IF \ CR TAB	   DUP ." .global " MCOUNT GTYPE 
		   CR DUP MCOUNT GTYPE ." :"
	     THEN DROP
	ELSE RDROP DROP
	THEN
