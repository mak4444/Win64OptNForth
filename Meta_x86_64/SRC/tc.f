...
\- SLITERAL : SLITERAL   STATE @ IF SLIT, THEN ; IMMEDIATE
...
: THERE?A   'FSTART - $30000  U< ;


\ : THERE?B DUP  THERE?A SWAP   IATBL - $4000 U< OR ;

: TTH  ['] THERE?A TO THERE? ;
...
: COUNT_SEG_CLOSE
 COUNT_SEG
 IF	HERE COUNT_SEG M!  0 TO COUNT_SEG
 THEN ;
...
\- &INTERPRET ' TC-INTERPRET  TO INTERPRET

\+ &INTERPRET ' TC-INTERPRET &INTERPRET !
[THEN]
...
: VIEW_S ( -- )		HERE TVIEWLINK , TO TVIEWLINK  0 ,  ;
: S_VIEW_PATH ( -- )	HERE TVIEWLINK , TO TVIEWLINK -1 , PARSE-NAME S", ;

: TALIGN  BEGIN  HERE $7 AND WHILE 0x90 C, REPEAT ;
...
: ZSTR>, ( <name> --  ) PARSE-NAME HERE SWAP DUP 1+ IALLOT MOVE ;
: NFUNK> ( <name> --  ) 0 W,  ZSTR>, ;
: ,NEXTFUNK ( adr -- adr' ) DUP TIMAGE-BASE - , 2+ ZCOUNT + 1+ ;
: NFUNKS ( adr adr-end -- )
   SWAP
  BEGIN ,NEXTFUNK
	2DUP U<=
